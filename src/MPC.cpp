...
// N is the number of timesteps in the horizon. dt is how much time
// elapses between actuations. For example, if N were 20 and dt were
// 0.5, then T would be 10 seconds.
size_t N = 10;
double dt = 0.1;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // the Solver function below.
     ...
     // ** from mpc_to_line sample code **

     // The cost is stored is the first element of `fg`.
     // Any additions to the cost should be added to `fg[0]`.
     fg[0] = 0;

#define WEIGHT_ON_COST
#ifdef WEIGHT_ON_COST
    // Weights for how "important" each cost is - can be tuned
    constexpr int cte_cost_weight = 2000;
    constexpr int epsi_cost_weight = 1000;
    constexpr int v_cost_weight = 1;
    constexpr int delta_cost_weight = 10;
    constexpr int a_cost_weight = 10;
    constexpr int delta_change_cost_weight = 100;
    constexpr int a_change_cost_weight = 10;
    ...
    for (int t = 0; t < N-2; t++) {
     ...
     for (int t = 1; t < N; t++) {
        ...
        for (int i = 1; i < coeffs.size(); i++) {
           psi_des0 += i*coeffs[i] * CppAD::pow(x0, i-1); // f'(x0)
        }
